...
public class disasmXE {
    ...
    private int disassembleInFormat3(opcode code, String opcodeHex,String objectcodeHEX,String addressHEX,int row, int current) throws IOException{
        String opcodeHEX = hteRecords.get(row).substring(current,current+2);
        String opcodeMnemonic = code.getOpMnemonic(opcodeHex);
        String calculatedAddressH , symbol = null;

        if(opcodeHEX.equals("00")){
            if(Integer.parseInt(objectcodeHEX,16)%3 == 0) {
                programC.write(pc.getPcCounter() + "\t" + sym.checkSYMTAB(pc.getPcCounter()) + "\tWORD\t" + Integer.parseInt(objectcodeHEX, 16) + "\t" + objectcodeHEX + "\n");
                pc.incrementPC_Word();
            }
            else{
                programC.write(pc.getPcCounter()+"\t"+sym.checkSYMTAB(pc.getPcCounter())+"\tBYTE\t"+Integer.parseInt(objectcodeHEX,16)+"\t"+objectcodeHEX+"\n");
                pc.incrementPC_Byte(objectcodeHEX.length()/2);
            }
        }else {
            if(opcodeMnemonic.equals("RSUB"))
                programC.write(pc.getPcCounter()+"\t"+sym.checkSYMTAB(pc.getPcCounter())+"\t"+opcodeMnemonic+"\t\t"+objectcodeHEX+"\n");
...
//                System.out.println("test "+Integer.toString(Integer.parseInt(calculatedAddressH,16)));
                ...
                programC.write(pc.getPcCounter()+"\t"+sym.checkSYMTAB(pc.getPcCounter())+"\t"+opcodeMnemonic+"\t"+symbol+",x"+objectcodeHEX+"\n");
    ...
    private void headerRecordDisassembler(int row) throws IOException {
        ...
        pc.initializePC(Integer.parseInt(startingAddress,16));
    ...
    private void textRecordDisassembler(int row) throws IOException {
        System.out.println(hteRecords.get(row));
        System.out.println(hteRecords.get(row).substring(1, 7));
...
//                System.out.println("StartingAddress: " + startingAddress);
                ...
                System.out.println("Error in disassembling instruction at "+e);
                break;
            }
        }
    }
